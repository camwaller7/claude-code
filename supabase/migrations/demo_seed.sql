...
insert into messages (conversation_id, direction, body, sent_at) values
  ((select id from conversations where external_thread_id = 'demo_x_001'), 'inbound', 'Hey! We''d love to send you our new pre-workout to review. Paid partnership — $1,500 for one post + story. Interested?', now() - interval '2 days'),
  ((select id from conversations where external_thread_id = 'demo_x_001'), 'outbound', 'Sounds great! I''d love to try FitFuel. Can you send over a brief so I can review the requirements?', now() - interval '1 day'),
  ((select id from conversations where external_thread_id = 'demo_x_001'), 'inbound', 'Absolutely! Brief is on its way. We''d need the post live by the 30th. Let us know if the timeline works!', now() - interval '3 hours');
...
with c as (
  ...
  values ('gmail', 'demo_gmail_001', 'Priya Kapoor', 'priya@luxelifestyle.com', 'brand_deal', 'needs_reply', 1, now() - interval '30 minutes', 'Partnership Proposal — LuxeLifestyle x You')
  returning id
)
insert into messages (conversation_id, direction, body, sent_at)
values ((select id from conversations where external_thread_id = 'demo_gmail_001'), 'inbound',
  'Hi there,
...
insert into messages (conversation_id, direction, body, sent_at) values
  ((select id from conversations where external_thread_id = 'demo_ig_003'), 'inbound', 'Your last reel was SO good! How long does it take you to edit one of those?', now() - interval '2 days'),
  ((select id from conversations where external_thread_id = 'demo_ig_003'), 'outbound', 'Thank you so much!! Usually about 3–4 hours for a good one 😊 The transitions take forever haha', now() - interval '1 day');
...
insert into messages (conversation_id, direction, body, sent_at)
values ((select id from conversations where external_thread_id = 'demo_gmail_002'), 'inbound', 'Congratulations! You have been selected to receive a free iPhone 15. Click here to claim your prize...', now() - interval '3 days');
...
insert into deals (brand_name, contact_name, status, deal_value, currency, notes, created_at) values
  ...
  ('Bloom Coffee Co.', 'David Park', 'sent_invoice', 2500, 'USD', 'Completed! 2 reels + 3 stories. Invoice sent, awaiting payment.', now() - interval '1 week'),
...
insert into clients (name, handle, product_purchased, purchase_date, status, notes, created_at) values
  ...
  ('Sophie Green', '@sophieg', 'Content Creator Masterclass', '2026-05-15', 'completed', 'Finished the course! Left a great testimonial.', now() - interval '38 days'),
  ('Marcus Webb', '@marcuswebb', '1:1 Coaching Session', '2026-05-20', 'completed', 'Grew from 2k to 8k followers after coaching.', now() - interval '33 days');
...
insert into posts (caption, hashtags, platforms, status, scheduled_at, published_at, created_at) values
  ...
  (
    ...
    array['instagram', 'facebook', 'threads', 'x'],
    'draft',
    null,
    null,
